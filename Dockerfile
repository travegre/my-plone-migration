...
# Install wheel first so sdists can be built, then pin setuptools exactly to
# what Plone 5.2.14 versions.cfg requires (65.7.0) and use zc.buildout 3.0.1
RUN pip install --no-cache-dir "wheel" && \
    pip install --no-cache-dir "setuptools==65.7.0" "zc.buildout==3.0.1"
