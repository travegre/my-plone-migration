...
def requested_queries(argv):
    # ``bin/instance run src/foo.py`` may leave both the script path and
    # interpreter/run arguments in sys.argv. Ignore all .py paths and options.
    queries = []
    for arg in argv[1:]:
        if not arg or arg.startswith('-') or arg.endswith('.py'):
            continue
        queries.append(arg)
