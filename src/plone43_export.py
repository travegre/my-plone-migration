...
The exporter uses each site's catalog as the authoritative active content set.
It never modifies the ZODB. Objects are written one-per-line to JSONL; binary
field payloads are written separately.
"""
...
def normalize_json(value):
    ...
    if isinstance(value, dict):
        return dict((normalize_json(k), normalize_json(v)) for k, v in value.items())
...
def json_line(value):
    rendered = json.dumps(normalize_json(value), ensure_ascii=False, sort_keys=True)
...
def parse_output_dir(argv):
    ...
    if len(args) == 1 and args[0].startswith('--output-dir='):
        ...
        raise SystemExit('Usage: bin/instance run src/plone43_export.py --output-dir=/path/to/export')
    if not outdir:
...
def field_export(obj, field, binary_dir, site_name, errors):
    ...
    try:
        ...
        try:
            ...
            errors.append({'site': site_name, 'path': object_path(obj), 'field': name,
...
def export_object(obj, site, site_name, binary_dir, errors):
    ...
    if callable(schema_method):
        try:
            ...
            errors.append({'site': site_name, 'path': object_path(obj),
...
def catalog_objects(site, errors, site_name):
    brains = list(site.portal_catalog.unrestrictedSearchResults())
    ...
    for brain in brains:
        ...
        try:
            ...
            errors.append({'site': site_name, 'path': path,
