...
semantics used by collective.easyform's pfg-migration-plone4 branch, but does
not install collective.easyform or its Plone-5-era dependency stack into the
Plone 4.3 buildout.
"""
...
def field_properties(obj):
    ...
    for field in obj.Schema().fields():
        name = field.getName()
        try:
            props[name] = field.getEditAccessor(obj)()
        except Exception:
            try:
...
def append_pfg_field(schema, obj):
    ...
    target_type = mapping.get(obj.portal_type)
    if target_type is None:
        raise ValueError('Unsupported PFG field type: %s' % obj.portal_type)
    ...
    for source_name, target_name in (
        ('fgTDefault', 'TDefault'), ('fgTEnabled', 'TEnabled'),
        ('fgTValidator', 'TValidator'), ('hidden', 'THidden'),
...
def fix_model_expressions(model):
    ...
    replacements = (
        ...
        (b"member and member.id or ''", b"member and member.getProperty('id', '') or ''"),
    )
        ...
        for site_name in SITES:
            ...
            for brain in brains:
                try:
                    ...
                    errors.append({'site': site_name,
                                   'source_path': getattr(brain, 'getPath', lambda: None)(),
                                   'operation': 'export_form', 'error': repr(exc),
                                   'traceback': traceback.format_exc()})
    rendered = json.dumps(normalize(errors), ensure_ascii=False, indent=2, sort_keys=True)
    if isinstance(rendered, unicode_type):
        rendered = rendered.encode('utf-8')
